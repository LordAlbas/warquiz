...
public class Statistiques_Admin extends JPanel implements MouseListener, MouseMotionListener, ItemListener{
	...
	/**
	 * R��cup��re le num��ro et le score de chaques quiz que le joueur �� fait.
	 * @throws SQLException
	 */
	public void Score(){
		try{
	        ...
	        int cpt = 200;
	        while(rs_score.next()){
	        	db_score = rs_score.getString("SCORE_USR_QUIZ"); // on r�cup�re le score
	        	db_num_quiz = rs_score.getString("ID_QUIZ"); // on r�cup�re le num quiz
	...
	/**
	 * R�cup�re le num�ro et le score des quiz 1:FACILE 2:MOYEN ou 3:DIFFICILE que le joueur � fait.
	 * @throws SQLException
	 */
	public void ScoreDifficulte(int diff){
		try{
	        ...
	        while(rs_score_diff.next()){
	        	db_score_diff = rs_score_diff.getString("SCORE_USR_QUIZ"); // on r�cup�re le score
	        	db_num_quiz_diff = rs_score_diff.getString("ID_QUIZ"); // on r�cup�re le num quiz
	        	//db_diff = rs_score_diff.getString("DIFFICULTE_QUIZ"); // on r�cup�re la difficulte
	...
	public int scoreMoyen(int id_quiz){
		...
		try{
	        ...
	        while(rs_moyenne.next()){
	        	score_moyen = rs_moyenne.getInt("score_moyen"); // on r�cup�re la moyenne
	        }
		} catch(ClassNotFoundException e){
			e.printStackTrace();
		}	
	...
	public void scoreMoyenTotal(){
		try{
	        ...
	        while(rs_moyenne_total.next()){
	        	db_moyenne_total = rs_moyenne_total.getString("score_moyen_total"); // on r�cup�re la moyenne
	...
	/**
	 * Calcule le nombre de parties jou�es par l'utilisateur.
	 * @throws SQLException
	 */
	public void nbPartiesJouees(){
		try{
	        ...
	        while(rs_nb_parties.next()){
	        	db_nb_parties = rs_nb_parties.getString("nb_parties"); // on r�cup�re la moyenne
	        	nb_parties = new JLabel("Vous avez jou� un total de " + db_nb_parties + " parties.");
	...
	/**
	 * Calcule le nombre de quiz jou� parmis le nombre max.
	 * @throws SQLException 
	 */
	public void nbQuizJouees(){
		try{
	        ...
	        while(rs_quiz_joue.next()){
	        	db_quiz_joue = rs_quiz_joue.getString("NB_QUIZ_JOUE"); // on r�cup�re la moyenne
	        }
	        while(rs_nb_quiz.next()){
	        	db_nb_quiz = rs_nb_quiz.getString("nb_quiz"); // on r�cup�re la moyenne
	        	nb_quiz = new JLabel("Nombre de quiz jou�(s) : " + db_quiz_joue + " / " + db_nb_quiz);
	...
	/**
	 * Le nombre de personnes qui on paticip� � un quiz donn�
	 * @param num_quiz
	 * @throws SQLException
	 */
	public int nbParticiants(int id_quiz){
		...
		try{
	        ...
	        while(rs_nb_participants_quiz.next()){
	        	nb_participants_quiz = rs_nb_participants_quiz.getInt("nb_participants_quiz"); // on r�cup�re le nombre       	
	        }
